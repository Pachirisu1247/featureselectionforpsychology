...
This may be a pretty good model. The fact that the \$incMSE is approaching reasonable numbers like 6 instead of 1 or 2 means we're starting to run out of questionable predictors. This might be a good parsimonious model, but in terms of strict prediction, the original model might still be the best given the modest sample size and what seem to be mostly weak predictors. However, we may not need all those measures of anxiety, and like we said earlier, we likely are looking for a 2-5 variable model (on the lower end, I'd expect) to cover almost all of the variance. So we should keep going before we start prediction.
...
R_squared <- 1 - (SSE / SST)
R_squared
#R2 0.077007 or 0.000105381 (this seems cap, make sure this is real)
#RMSE 6.4205	
