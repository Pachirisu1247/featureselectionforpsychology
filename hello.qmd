...
Random Forest is a popular machine learning method for classification and regression, particularly if you are interested in sheer predictive performance. Random Forests are built on decision trees as the basic "unit" underlying analysis. Random Forest constructs many decision trees for various subsets of the dataset (the namesake of "random forest" of trees), then aggregates the results of the trees to produce a more robust estimate. Although random forests do not seek to optimize a loss function in the exact same way regression models do, they naturally do attempt to minimize "error" when creating the decision trees - in the case of regression, which we are discussing today, the mean square error.

In this document, I will walk through a random forest prediction procedure for a psychological dataset. The dataset is extremely characteristic of many psychology datasets - it has an okay but not at all large sample size (735 N before being split into test/training sets), a relatively high number of potential predictors (20+ collected as part of the survey, many of which are likely completely unhelpful in prediction), and what is likely middling effect sizes for predictors. We'll review multiple different ways of creating models, selecting features, and evaluating the models.
...
General state anxiety/score_STAI_state_short, as assessed by the sum of the five-item scale STAI-SKD (Englert et al. 2011)
...
##View(amatus). We can use this command to view our data.
 #WHY WAS THERE AN AMATUSCLEAN2? LOOK!
...
Our data is clearly skewed right. There's no missing data for our Y variable of performance since we are not interested in participants who answered out of order against instructions.
...
set.seed (40)#I'm going to put this before every random generation just for clarity.
rf_mtry2<-train(sum_arith_perf~scale(score_AMAS_total)+sex+scale(age)+scale(score_BFI_N)+scale(score_GAD)+scale(score_PISA_ME)+scale(score_STAI_state_short)+scale(score_TAI_short)+scale(score_SDQ_M)+scale(score_SDQ_L), data=training, importance=TRUE, method="rf", tuneGrid=tunegrid,trControl=control) ##Notice we keep using the importance=TRUE parameter, which calculates variable importance metrics. We need this to be set to TRUE to be able to see varimp plots later. 
...
rf_firstselection <- train(sum_arith_perf~scale(score_AMAS_total)+sex+scale(age)+scale(score_BFI_N)+scale(score_GAD)+scale(score_PISA_ME)+scale(score_STAI_state_short)+scale(score_SDQ_M)+scale(score_SDQ_L), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
print(rf_firstselection)
#we'll look at variable importance again in a minute


####TESTING ACCURACY OF MODEL ON TEST SET! DON'T USE THIS FOR VARIABLE SELECTION BECAUSE ITS UNSEEN DATA!
predictionsfirstselection<-predict(rf_firstselection,test)
predictionsfirstselection<-data.frame(R2 = R2(predictionsfirstselection, test $ sum_arith_perf), 
		RMSE = RMSE(predictionsfirstselection, test $ sum_arith_perf), 
		MAE = MAE(predictionsfirstselection, test $ sum_arith_perf))#these statistical functions are in caret technically under trainControl(), which passes this argument through defaultSummary() I believe.
print(predictionsfirstselection)
...
Notice that the %incMSE of the lowest variable is not close to 1 or 2, as test anxiety was. It's higher. Our model may be slightly more parsimonious.
...
rf_secondselection <- train(sum_arith_perf~scale(score_AMAS_total)+sex+scale(age)+scale(score_BFI_N)+scale(score_STAI_state_short)+scale(score_PISA_ME)+scale(score_SDQ_M)+scale(score_GAD), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
print(rf_secondselection)


####TESTING ACCURACY OF MODEL ON TEST SET! DON'T USE THIS FOR VARIABLE SELECTION BECAUSE ITS UNSEEN DATA! JUST USE IT IF YOU'RE INTERESTED!
predictionssecondselection<-predict(rf_secondselection,test)
predictionssecondselection<-data.frame(R2 = R2(predictionssecondselection, test $ sum_arith_perf), 
...
print(predictionssecondselection)

```
```{r}
varImpPlot(rf_secondselection$finalModel, type=1)#type 1 refers to estimates following perturbation. 
```
Slight reduction in RMSE and increase in (training data) R-squared! More importantly, the variable importance metrics look a lot better - the lowest incMSE% is around 5%. great news for our goal of parsimonious modeling. However, I'm not sure if this will continue. It's okay if while going through variable selection, your model doesn't necessarily have a massive reduction in RMSE or massive increase in r squared. Remember that we're looking to explain the most variance with as few variables (parsimony), so sometimes parsimony is worth slight decreases in predictive accuracy (though you should make sure you're not cutting a seemingly low-predictive power variable that nonetheless has explanatory importance).

This may be a pretty good model. The fact that the $incMSE is approaching reasonable numbers like 6 instead of 1 or 2 means we're starting to run out of questionable predictors. This might be a good parsimonious model. However, we may not need all those measures of anxiety, and like we said earlier, we likely are looking for a 2-5 variable model (on the lower end, I'd expect). So we should keep going before we start prediction.

Like we said, those measures of anxiety seem suspect. I'll cut state anxiety next. 

```{r}
#| warning: false
#| label: Third selection
set.seed (43)#I'm going to put this before every random generation just for clarity.
rf_thirdselection <- train(sum_arith_perf~scale(score_AMAS_total)+sex+scale(age)+scale(score_BFI_N)+scale(score_PISA_ME)+scale(score_GAD)+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
predictionsthirdselection<-data.frame(R2 = R2(predictionsthirdselection, test $ sum_arith_perf), 
...
print(predictionsthirdselection)
...
#|
set.seed(44)
rf_fourthselection <- train(sum_arith_perf~scale(score_AMAS_total)+sex+scale(age)+scale(score_BFI_N)+scale(score_PISA_ME)+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
predictionsfourthselection<-data.frame(R2 = R2(predictionsfourthselection, test $ sum_arith_perf), 
...
print(predictionsfourthselection)

varImpPlot(rf_fourthselection$finalModel, type=1)#type 1 refers to estimates following perturbation. 

```
See how the RMSE went a little back down again? To be expected.
Our hunch earlier was correct - looks like the anxiety variables aren't really helping us all that much. I suspect the MOST parsimonious model won't have these either.
Let's cut the last one then.

```{r}
#| warning: false
#| 
set.seed(45)
rf_fifthselection <- train(sum_arith_perf~sex+scale(age)+scale(score_BFI_N)+scale(score_PISA_ME)+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
predictionsfifthselection<-data.frame(R2 = R2(predictionsfifthselection, test $ sum_arith_perf), 
...
print(predictionsfifthselection)
```

```{r}
varImpPlot(rf_fifthselection$finalModel, type=1)#type 1 refers to estimates following perturbation. 
```
Woah, this model looks way better. RMSE is way down and R squared is up. In fact, this seems like it may even be better than our 8-variable model. We'll keep this in mind as we cut neuroticism, which again seems to be an extremely irrelevant predictor. However, keeping neuroticism may be useful for predictive power of our parsimonious model.
Also, if you haven't noticed, age used to be one of the predictors with the lowest incMSE% in the full model, but has stuck in the models we are testing. 
...
#| 
set.seed(46)
rf_sixthselection <- train(sum_arith_perf~sex+scale(score_PISA_ME)+scale(score_SDQ_M)+scale(age), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
```
The MSE went up a bit again, but clearly we're honing in on a good parsimonious model. At this point, it seems safe to say that the four variables above are the most "important" variables in terms of building a parsimonious model, and math self-efficacy and sex are likely the top two. Given the better metrics of the fifth selection model, neuroticism also seems to be a pretty good predictor in a parsimonious model, and we likely don't want to cut it. However, you may only want to identify the "most important variables", and though neuroticism may be a good predictor in a 5 variable model, it may not be the "most important variable".
The 4 or 5 variable models seem to be pretty good parsimonious models if we want to collect more than only 2 variables. In other words, these 5 variables can produce a good model that captures a good amount of the variability in scores, but this model is likely neither the most parsimonious (that seems to be a model with only a couple predictors, as we've indicated before) nor the most predictive (that would be the model with the raw best predictive power, which we'll test soon).

Instead of just cutting the last few variables, I will provide an example of an explicit method for variable selection in random forest models that will return the same output. VSURF, the method in question, essentially uses backward stepwise selection followed by forward selection based on variable importance (using a basic definition as we used above being, depending on the task, the difference between MSE/misclassification rate after permuting each predictor). There have been several methods for variable selection described in the literature, however, a recent simulation study demonstrated that VSURF often demonstrates some of the best performance for classification tasks (Speiser et al.), which may extend to regression. However, it is worth noting that VSURF is prediction oriented and as such, accepts the risk of producing false negatives.

Instead of cutting the last few variables, I'll 
...
rm(x)
set.seed(46)
vamatus<-VSURF(sum_arith_perf~score_AMAS_total+score_PISA_ME+age+score_BFI_N+score_GAD+sex+score_STAI_state_short+score_TAI_short+score_SDQ_M+score_SDQ_L, data=amatusclean, na.action=na.omit)
summary(vamatus)
plot(vamatus, step = "pred", imp.sd = FALSE, var.names = TRUE)

(vamatus$terms)
vamatus$varselect.pred
...
#| 
set.seed(47)
rf_strongestvariables <- train(sum_arith_perf~sex+scale(score_PISA_ME)+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
set.seed(48)
rf_strongestvariables2 <- train(sum_arith_perf~sex+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
set.seed(49)
rf_strongestvariables3 <- train(sum_arith_perf~scale(score_PISA_ME)+scale(score_SDQ_M), data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
set.seed(50)
rf_strongestvariables4 <- train(sum_arith_perf~scale(score_PISA_ME)+sex, data=training, method="rf", importance=TRUE, tuneGrid=tunegrid, trControl=control)
...
```
As you can see, the model with sex and self-efficacy has by far the lowest MSE and the best R-squared. Its r-squared is comparable to models with much more, and its MSE isn't too. It's safe to say that this is definitely the most parsimonious model in the sense that it explains a ton of the variance with only two variables. Adding a couple more variables will likely increase raw predictive power, but if you're looking to identify the model with the absolute best predictive power in the fewest variables, look no further.

Prediction time. Let's see if my entire paper has been bullshit or not.
```{r}
#| label: Prediction
####TESTING ACCURACY OF MODEL ON TEST SET!
#First, we'll make sure we have a spare test set.
test2<-test
#first, find scaling parameters.

scale(training$score_BFI_N) #mean 23.48256, scale 6.17619
scale(training$score_AMAS_total)#18.27326, 6.39728
scale(training$age) #23.6357, 3.880785
scale(training$score_GAD) #12.47287, 3.950819
scale(training$score_PISA_ME)#20.5155, 3.357321
scale(training$score_STAI_state_short) #8.459302, 2.898324
scale(training$score_TAI_short)#11.10853, 3.849076
scale(training$score_SDQ_L)#13.75775, 2.404171
scale(training$score_SDQ_M)#11.62209, 3.030367
...
scaling_params <- list(
  score_BFI_N = list(mean = 23.48256, sd = 6.17619),
  score_AMAS_total = list(mean = 18.27326, sd = 6.39728),
  age = list(mean = 23.6357, sd = 3.880785),
  score_GAD = list(mean = 12.47287, sd = 3.950819),
  score_PISA_ME = list(mean = 20.5155, sd = 3.357321),
  score_STAI_state_short = list(mean = 8.459302, sd = 2.898324),
  score_TAI_short = list(mean = 11.10853, sd = 3.849076),
  score_SDQ_L = list(mean = 13.75775, sd = 2.404171),
  score_SDQ_M = list(mean = 11.62209, sd = 3.030367)
)
...
for (predictor in names(scaling_params)) {
  if (predictor %in% colnames(test)) {  # Check if the predictor exists in the dataset
    ...
    # Calculate the mean and standard deviation of the current predictor
    old_mean <- mean(test[[predictor]], na.rm = TRUE)
    old_sd <- sd(test[[predictor]], na.rm = TRUE)
    
    # Overwrite the original predictor with the scaled version
    test[[predictor]] <- ((test[[predictor]] - old_mean) / old_sd) * new_sd + new_mean
  } else {
...
predictionsmtry2 <- predict(rf_mtry2, newdata = test)  # Corrected prediction line
predictionsmtry2 <- predict(rf_mtry2, newdata = test2)  # Trying a second prediction line
View(predictionsmtry2)
...
predictionsvsurfmodel<-predict(rf_strongestvariables4, test $ sum_arith_perf)
# model performance metrics ?

testmtry2<-data.frame(R2 = R2(predictionsmtry2, test $ sum_arith_perf), 
		RMSE = RMSE(predictionsmtry2, test $ sum_arith_perf), 
		MAE = MAE(predictionsmtry2, test $ sum_arith_perf))
print(testmtry2)

testfirstselection<-data.frame(R2 = R2(rf_firstselection, test $ sum_arith_perf), 
		RMSE = RMSE(predictionsfirstselection, test $ sum_arith_perf), 
		MAE = MAE(predictionsfirstselection, test $ sum_arith_perf))
print(testfirstselection)

View(predictionsmtry2)
actuals <- test$sum_arith_perf
View(actuals)

summary(test$sum_arith_perf)


#seems like the default mtry (as opposed to mtry=2) tends to have slightly better rsquared but worse standard error for the test set. Perhaps there's an mtry in between 2 and 3.333 that may have a nicer balance of the two?  
...
actuals <- test$sum_arith_perf
SSE <- sum((predictionsmtry2 - actuals)^2)  # Sum of Squared Errors
SST <- sum((actuals - mean(actuals))^2)  # Total Sum of Squares
R_squared <- 1 - (SSE / SST)
R_squared
